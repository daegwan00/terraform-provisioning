resource "aws_iam_role" "gitaction" { # 역할 생성 
  name               = "gitaction"
  path               = "/"
  ...
  statement {
    ...
    principals {
      ...
      identifiers = [
        ...
        # oidc로 인증받은 깃 허브 액션 컨테이너가 이 롤을 사용할 수 있도록 federated로 설정한다
      ]
    }
    ...
    condition {
      ...
      values = [
        "repo:daegwan00/springboot-sample:*",
      ]
    }
...
resource "aws_iam_role_policy" "gitaction_ecr" { # ecr policy 생성
  name   = "gitaction-ecr"
...
# resource "aws_iam_role_policy" "gitaction_ecs" {   # ecs policy 생성
#   name   = "gitaction-ecs"
#   role   = aws_iam_role.gitaction.id
